...
class GoogleDataset(InputDataset):
    ...
    def read_ann_csv(self, target):
        ...
        for img_id in all_img:
            if img_id in img_to_delete:
                del ann_dict[img_id]
            elif len(ann_dict[img_id].keys()) == 1 and ann_dict[img_id][0]["normalized_area"] < target.min_normalized_bbox_area:
                ...
                del ann_dict[img_id]
            elif target.classification and len(ann_dict[img_id].keys()) > 1:
                del ann_dict[img_id]
        return ann_dict
    ...
    def subsample(self, target, N, policy="random"):
        ...
        self.intersecting_g_class, self.intersecting_target, self.intersecting_g_idx, self.mapping_g_target = self.intersecting_classes(target=target)
        ...
        if self.type == "imagenet" or self.type == "ade20k":
